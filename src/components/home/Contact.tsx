...
export default function Contact() {
  ...
  return (
    <>
      {/* Success Message */}
      {showMessage && (
        <p className="text-green-500 border border-gray-100 font-medium px-4 py-2 rounded-xl shadow-sm text-sm">
          Congratulations message received 🎉🎉🎉
        </p>
      )}
      {/* Error Message */}
      {showErrorMessage && (
        <p className="text-red-500 border border-gray-100 font-medium px-4 py-2 rounded-xl shadow-sm text-sm">
      ...
      >
        ...
        <button
          ...
          className="bg-linear-to-r from-[#7c3bed] to-[#3c83f6]  text-white w-full text-sm font-medium py-3 rounded-xl hover:brightness-95 hover:cursor-pointer duration-300 ease-out outline-none"
        >
          {state.submitting ? (
